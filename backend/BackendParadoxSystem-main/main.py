...
async def add_user(
    ...
    try:
        ...
        data = await request.json()
        insert_query = f"INSERT INTO users(username, password, user_control, email, sales, sales_return, purshase, purshase_return, orders, trans, items, chart, statement, SAType, Branch, COH, EOD, RecallInv,TableUser,LockAfterDone,Pos,ManageItems,ManageGrpItems,InvHistory,DailySales,CashOnHands,Description,UnitPrice,EndshiftEod,EndShiftCOH,AddandupdateSection,AddandUpdateTable,BarCodeUpdate) VALUES (%s,%s ,%s,%s, %s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        cursor.execute(insert_query, (user_name_uppercase, '', '', '', "N", "N", "N", "N", "N", "N", "N", "N", "N", "SA", '', user_name_uppercase, "N", "N","N", "N","N", "N","N", "N","N", "N","UPrice","1","Y", "Y","Y", "Y","Y"))
...
#         # The connection will be automatically closed when it goes out of scope
#         pass
@app.get("/pos/getPrinters")
async def get_printers():
    try:
        # Get a list of all printers
        printers = win32print.EnumPrinters(win32print.PRINTER_ENUM_LOCAL | win32print.PRINTER_ENUM_CONNECTIONS)
        
        # Extract printer names from the list
        printer_names = [printer[2] for printer in printers]  # printer[2] contains the printer name
        
        return {"printers": printer_names}
    except Exception as e:
        print("Error details:", str(e))
...
async def post_invoiceitem(company_name: str, request: Request):
    try:
        ...
        if data["closeTClicked"]:
            ...
            cursor.execute(f"Select KT, PrinterName,ReportName from printers")
            printer_data = cursor.fetchall()
            printer_dic = {key: {"name": name, "report_name": report_name} for key, name, report_name in printer_data}
            # Specify keys for grouping
            keys_to_group_by = ['KT1', 'KT2', 'KT3', 'KT4']
            inv_num = ''
            order_id = ''
            ...
            # Group meals by printer names
            for meal in data["unsentMeals"]:
                printer_names = [printer_dic.get(meal[kt_key], {}).get("name", "") for kt_key in keys_to_group_by]
                # Remove dashes from printer names
                printer_names = [name.replace('-', '') for name in printer_names]
                non_empty_printer_names = [name for name in printer_names if name]  # Filter out empty strings
                if non_empty_printer_names:  # Check if there are non-empty printer names
            ...
            if data["tableNo"]:
                ...
                conn.commit()
            return {"message": "Invoice items added successfully", "selectedData": items_by_kitchen, "invoiceDetails": invnum_dicts,"reportNames": [printer_dic.get(meal[kt_key], {}).get('report_name') for meal in data["unsentMeals"] for kt_key in keys_to_group_by], "printerNames": [printer_dic.get(meal[kt_key], {}).get('name') for meal in data["unsentMeals"] for kt_key in keys_to_group_by] }
...
async def print_receipt(request: Request):
    try:
        ...
        printable_content = data.get("content")
        printer_name = data.get("printerName")
        

        # Validate that printer exists
        printers = win32print.EnumPrinters(win32print.PRINTER_ENUM_LOCAL | win32print.PRINTER_ENUM_CONNECTIONS)
        printer_names = [printer[2] for printer in printers]

        if printer_name not in printer_names:
            raise HTTPException(status_code=400, detail=f"Printer '{printer_name}' not found.")

        # Convert HTML content to plain text
        text_content = html2text.html2text(printable_content)  # Can enhance if needed
        ...
        # Print content with line spacing (adjust as needed)
        line_spacing = 100
        for i, line in enumerate(text_content.splitlines()):
            dc.TextOut(100, 100 + i * line_spacing, line)
        ...
        raise HTTPException(status_code=500, detail=f"Printing error: {str(e)}")
...
@app.get("/pos/reportUserShiftEod/{company_name}/{date}/{allowedUser}")
async def getReportUserShiftEod(company_name: str, date:str, allowedUser: str):
    try:
        conn = get_db(company_name)
        cursor = conn.cursor()
        formatted_date = date.replace('.', '/')
        ...
        JOIN 
            inv ON inv.InvNo = invnum.InvNo
        WHERE 
             (invnum.EOD = '' or invnum.EOD is null)AND (invnum.User IS NOT NULL AND invnum.User != '')
        """
        if allowedUser != "All":
            query = base_query + f" AND invnum.User = '{allowedUser}' GROUP BY invnum.InvNo"
        else:
...
@app.get("/pos/getLockAfterDoneRecallInv/{company_name}/{username}")
async def getLockAfterDoneRecallInv(company_name: str, username: str):
    try:
        conn = get_db(company_name)
        cursor = conn.cursor()
        cursor.execute(f"SELECT LockAfterDone, RecallInv FROM users WHERE username='{username}' ")
        COHR = cursor.fetchone()
        return {
            "lockAfterDone": COHR[0],
            "recallInv": COHR[1]
        }
    except HTTPException as e:
        print("Error details:", e.detail)
...
@app.get("/pos/checkBarcodeUpdatePermission/{company_name}/{username}")
async def check_BarcodeUpdatePermission(company_name: str, username: str):
    conn = None
    try:
        conn = get_db(company_name)
        cursor = conn.cursor()
        
        # Correct SQL query with appropriate placeholder
        cursor.execute("SELECT BarCodeUpdate FROM users WHERE username = %s", (username,))
        result = cursor.fetchone()
        
        if result:
            return {"BarCodeUpdate": result[0]}
        else:
            return {"BarCodeUpdate": "N"}  # Default to "N" if the user is not found
    except Exception as e:
        print("Error details:", e)
        raise HTTPException(status_code=500, detail="Internal Server Error")
    finally:
