          ...
          if (responseData["message"] === "Invoice items added successfully") {
           ...
           if (allowPrintKT === "Y") {
            ...
            Object.entries(mealsByKitchen).forEach(([kitchen, meals], index) => {
              const reportNames = responseData.reportNames;
               const printerNames = responseData.printerNames;
                // Find the correct printer name for the kitchen
       
              let printableContent = '';
              
              switch (reportNames[index]) {
                case 'Report 1':
                  printableContent = `
                      <style>
                      ...
                      <div class="printable" style="text-align: center; page-break-after: always;">
                          <h1>Report 1 - Receipt for ${kitchen}</h1>
              ...
              case 'Report 2':
                  printableContent = `
                      <style>
                          .printable { display: none; }
                          @media print { .printable { display: block; background-color: #f4f4f4; padding: 20px; } }
                      </style>
                      <div class="printable" style="text-align: left; page-break-after: always;">
                          <h2>Report 2 - Receipt for ${kitchen}</h2>
                          <table style="width: 100%; border: 1px solid #ddd; padding: 5px;">
                              <thead>
                                  <tr>
                                      <th style="border: 1px solid #ccc; padding: 10px;">Item No</th>
                              ...
                              <tbody>
                                  ${meals.map(meal => `
                                      <tr>
                                          <td style="border: 1px solid #ccc; padding: 10px;">${meal.ItemNo}</td>
                                          <td style="border: 1px solid #ccc; padding: 10px;">${meal.ItemName}</td>
                                          <td style="border: 1px solid #ccc; padding: 10px;">${meal.quantity}</td>
                                      </tr>
                                  `).join('')}
                              </tbody>
                          </table>
              ...
              case 'Report 3':
                  printableContent = `
                      <style>
                          .printable { display: none; }
                          @media print { .printable { display: block; font-family: Arial, sans-serif; margin: 20px; } }
                      </style>
                      <div class="printable" style="page-break-after: always;">
                          <h3 style="text-align: center;">Report 3 - Kitchen Orders for ${kitchen}</h3>
                          <table style="width: 100%; border: 2px solid #333;">
                              <thead>
                                  <tr>
                                      <th style="border: 2px solid #333; padding: 12px;">Item No</th>
                                      <th style="border: 2px solid #333; padding: 12px;">Item Name</th>
                                      <th style="border: 2px solid #333; padding: 12px;">Quantity</th>
                                  </tr>
                              </thead>
                              <tbody>
                                  ${meals.map(meal => `
                                      <tr>
                                          <td style="border: 2px solid #333; padding: 12px;">${meal.ItemNo}</td>
                                          <td style="border: 2px solid #333; padding: 12px;">${meal.ItemName}</td>
                                          <td style="border: 2px solid #333; padding: 12px;">${meal.quantity}</td>
                                      </tr>
                                  `).join('')}
                              </tbody>
                          </table>
                  ...
                  break;
              default:
                  // If no report is matched, use a default design
                  printableContent = `
                      <style>
                          .printable { display: none; }
                          @media print { .printable { display: block; } }
              ...
              setTimeout(async () => {
                try {
                  const printResponse = await fetch(`${url}/pos/print`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                      kitchen,
                      content: printableContent,
                      printerName: printerNames[index], // Ensure kitchen name corresponds to a valid printer
                    }),
                  });
          
                  if (!printResponse.ok) {
                    // Output response status and details to identify issues
                    const errorText = await printResponse.text();
                    console.error(`Failed to print receipt (Status ${printResponse.status}): ${errorText}`);
                  } else {
                    console.log(`Print job sent successfully for kitchen: ${kitchen}`);
                  }
                } catch (error) {
              ...
              printDelay += 1000; // Delay for each kitchen to avoid overload
            });
          }
